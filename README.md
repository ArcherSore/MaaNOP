...
> [!NOTE]
> **大区支持说明**：
> 目前已全面适配官方微端的 **「公测大区」** 与 **「不删档大区」** 全部分页！可在全局配置中自由切换。 
> *(注：联盟大区暂未适配)*
...
3. 确保电脑已安装 **火影忍者 Online 微端**（⚠️ **当前仅支持官方微端**，QQ游戏大厅、360游戏大厅均不支持）。
4. 双击运行 `NarutoAutoGUI.exe`。
...
- **注意事项**：该任务要求从全新角色创建起跑，中途异常停止 **暂不支持断点续跑**。【未来可能会支持，但希望不大】
...
### ⚔️ 竞技场一键六道 (`ArenaRankGrind`)
- **功能描述**：自动进入竞技场，连续进行正式挑战并处理战斗流程，直到当前账号达到六道段位。
- **使用方法**：
  1. 将游戏停留在竞技场主界面，或确保能够看到竞技场入口。
  2. 在任务页选择 **“竞技场一键六道”**，返回首页点击 **“开始任务”**。
- **执行流程**：程序会自动进入竞技场、检查当前段位、开始挑战、处理自动战斗和战斗结果，并在达到六道后结束任务。
- **重要注意事项**：该任务**不支持多账号刷**，只处理当前已登录的单个账号，不会自动切换或批量处理多个账号；如果账号已达到六道，任务会直接结束。
...
- **首次安装后的处理**：首次安装并打开程序后，如果遇到“准备运行环境失败”，请先完全退出程序并**重启电脑**，重启后再重新运行！！！
- **版本实测**：**Windows 专业版（Pro）开箱即用**（虚拟机已验证）；**家庭版（Home）** 目前在开发者实机和部分用户电脑成功运行。
