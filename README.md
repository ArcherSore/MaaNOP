...
> [!WARNING]
> # ⚠️ 注意：目前仅支持【公测】区服！
> **当前所有选服识别、OCR 匹配与自动化逻辑仅适配「公测」大区（例如公测 1 区、公测 xxx 区）！**  
> 其他大区暂未适配，请务必在游戏微端的**「公测」**分区下运行！【下版本适配】
...
3. 确保电脑已安装 **火影忍者 Online 微端**（⚠️ **当前仅支持官方微端**，且**仅支持「公测」大区**！QQ游戏大厅、360游戏大厅及其它大区均不支持）。
4. 双击运行 `NarutoAutoGUI.exe`（在 Windows 提示时允许 UAC 权限）。
5. 在首页点击 **“准备运行环境”**，等待系统拉起独立的 Child Session 后台桌面。
6. 在弹出的会话桌面中正常登录游戏，并停留在 **选服务器界面**（⚠️ **务必停留在「公测」大区分页**）。
...
### 🚪 前置选服与跨区支持（全局设置）
- ⚠️ **大区支持**：**目前仅支持「公测」大区！**（涵盖公测 1-999 区与 1000+ 区，非公测大区暂未适配）。【下版本适配】
- **服务器范围格式**：支持形如 `1-10,15,20-25` 的跨区区间配置，任务会按范围依次处理对应公测区服。
